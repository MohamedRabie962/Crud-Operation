...
<?php  $row = $db->find('users',$_GET['id']); ?>
<?php if(isset($_GET['id']) && is_numeric($_GET['id']) && $row):  ?>

<?php 
    $department = array("it","cs","com");
    $error = '';
    $success = '';
?>
        ...
        else 
        {
            if(strlen($name) > 3)
            {
                if(filter_var($email,FILTER_VALIDATE_EMAIL))
                {
                    ...
                    if(in_array($department, (array)$department))
                    {
                        if(!empty($password))
                        {
                        ...
                        else 
                        {
                            ...
                            $sql = "UPDATE users SET `name`='$name',`email`='$email',`department`='$department',
...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
            <h2 class="p-3 col text-center mt-5 text-white bg-primary">  Edit User </h2>
        </div>
